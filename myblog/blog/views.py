...
def post_new(request):
    ...
    #만들어놨던 폼양식을 가져오기 위해서는
    #변수=폼양식()를 써야한다. ()가 우축에 붙음에 주의
    #현재 코드는 PostForm() 양식을 따라 만들것임을 보여준다.
        form = PostForm()
    #저장한 폼 양식은 템플릿 파일에 같이 넘겨야한다.
...
def post_edit(request, pk):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            #만약 post방식이 아닌 경우라면 get 방식이므로 수정 직전임
            #따라서 폼으로 다시 연결해줘야함. 이때의 폼은 수정용 폼이며
